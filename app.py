...
try:
    ...
    st.sidebar.caption("Run diagnostics: `streamlit run pages/01_Mini_Diagnostics.py`")

for k in ["points_gdf","results_gdf","dem","awc","clc","slope"]:
...
def water_exec_summary(df: pd.DataFrame) -> str:
    ...
    # Dominant land cover name
    dom_lc = "Unknown"
    if "land_cover_name" in df.columns and n:
        dom_lc = df["land_cover_name"].fillna("Unknown").mode().iloc[0]
    summary = f"""
**Portfolio Water Summary**
- Sites analyzed: **{n}**
- Recharge classes: **{pct_low:.0f}% Low**, **{pct_med:.0f}% Medium**, **{pct_high:.0f}% High**
- Water stress flags: **{stress}** site(s)
- Near water (CLC water): **{near_w}** site(s)
- Dominant land cover: **{dom_lc}**
- Slope quality: **{slope_q}**
"""
    return summary
...
def render_charts(df: pd.DataFrame):
    ...
    # Slope vs AWC scatter, color by recharge class
...
def build_html_memo(df: pd.DataFrame) -> str:
    """Produce a simple, styled HTML memo with the executive summary and key stats."""
    n = len(df)
    pct_low  = (df["recharge_class"].eq("Low").mean()*100) if n else 0
    pct_med  = (df["recharge_class"].eq("Medium").mean()*100) if n else 0
    pct_high = (df["recharge_class"].eq("High").mean()*100) if n else 0
...
if page.startswith("1"):
    ...
    with c1: dem_up = st.file_uploader("DEM (.tif)", type=["tif","tiff"], key="dem")
    with c2: awc_up = st.file_uploader("AWC (.tif)", type=["tif","tiff"], key="awc")
    with c3: clc_up = st.file_uploader("CLC2018 (raster .tif OR vector .gpkg/.geojson/.shp)", type=["tif","tiff","gpkg","geojson","json","shp"], key="clc")
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                with st.spinner("Analyzing point(s)..."):
                    try:
                        ...
                        st.session_state["results_gdf"] = pd.DataFrame(out.drop(columns=["geometry"], errors="ignore"))
